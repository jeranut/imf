...
8. Enregistrement : déclenche les contraintes `@api.constrains` (section 7/8) ; en cas de succès, le produit devient sélectionnable lors de la création d'un crédit (workflow `dossier_precredit`, hors périmètre ici).
...
**Champs présents dans le modèle Python mais absents de toute vue (tree et form)** : `guarantee_required` (Garantie obligatoire) et `min_guarantee_ratio` (Ratio minimum de garantie (%)) ne sont affichés dans aucune vue actuelle du produit — ils existent et sont contraints par `@api.constrains` mais ne sont pas exposés à la saisie dans l'UI.
...
- `@api.constrains` `_check_values` (sur `min_amount`, `max_amount`, `min_term`, `max_term`, `interest_rate`, `grace_period_days`, `min_membership_days`, `min_guarantee_ratio`, `fee_amount`, `fee_rate`) : voir messages d'erreur exacts en section 8.
- `@api.constrains` `_check_repayment_frequency_mode` (sur `repayment_frequency_mode`, `repayment_frequency_id`, `allowed_repayment_frequency_ids`) : cohérence obligatoire entre le mode choisi et la périodicité renseignée.
- Valeurs par défaut calculées dynamiquement : tous les comptes PCEC via `_pcec_default(code)` (recherche `account.account` par `code` + `company_id` courant) et les journaux `disbursement_journal_id`/`payment_journal_id`/`fee_journal_id` via `_journal_default('CRE')` (recherche `account.journal` par code + société). Si le compte/journal n'existe pas pour la société (plan PCEC non chargé), le champ reste simplement vide.
- Méthode `_get_account(kind, partner)` : sélectionne dynamiquement le compte `account_<kind>_individuel_id` ou `account_<kind>_groupe_id` selon `partner.microfinance_client_type` (`'group'` → variante groupe, sinon → variante individuel). Utilisée par les écritures comptables du crédit (hors périmètre de ce README, voir workflow `comptabilite`).
...
- Comptes PCEC vides après création : le plan comptable PCEC n'est pas encore chargé pour la société (`_pcec_default` ne trouve aucun compte avec ce code pour `company_id`), les champs concernés restent vides et doivent être renseignés manuellement.
