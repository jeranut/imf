...
class HrPayslip(models.Model):
    ...
    date_to = fields.Date(string='Date To', required=True,
                          ...
                          default=lambda self: fields.Date.to_string(
                              (datetime.now() + relativedelta(months=+1, day=1,
                                                              days=-1)).date()))
    # Workflow : draft -> verify -> done -> paid (ou cancel depuis
    # draft/verify/done).
    state = fields.Selection(selection=[
        ...
        help="""* Brouillon : le bulletin vient d'être créé.
                \n* En attente : le bulletin a été calculé et est soumis à
                vérification.
                \n* Validé : le bulletin est confirmé (écriture comptable
                générée).
                \n* Payé : le bulletin a été réglé.
                \n* Rejeté : le bulletin a été annulé.""")
    ...
    move_id = fields.Many2one('account.move', string='Écriture comptable',
                              readonly=True, copy=False,
                              help="Écriture comptable générée à la "
    ...
    def action_payslip_verify(self):
        """Calcule le bulletin et le fait passer en état 'En attente'"""
        self.action_compute_sheet()
        return self.write({'state': 'verify'})

    def action_payslip_done(self):
        """Valide un bulletin en état 'En attente' (génère l'écriture
        comptable) et le fait passer en état 'Validé'"""
        if any(payslip.state != 'verify' for payslip in self):
            raise UserError(
                _("Seul un bulletin en état 'En attente' peut être validé."))
        for payslip in self:
            payslip._create_account_move()
        return self.write({'state': 'done'})

    def action_payslip_paid(self):
        """Marque un bulletin validé comme payé"""
        if any(payslip.state != 'done' for payslip in self):
            raise UserError(
                _("Seul un bulletin 'Validé' peut être marqué comme payé."))
        return self.write({'state': 'paid', 'paid': True})

    def _prepare_account_move_line_vals(self):
        """Ventile les lignes du bulletin (Basic/Allowance/Deduction/
        Company Contribution) par compte comptable, en s'appuyant sur les
        comptes débit/crédit configurés sur chaque hr.salary.rule."""
        self.ensure_one()
        category_codes = ('BASIC', 'ALW', 'DED', 'COMP')
        amounts = {}
        for line in self.line_ids:
            if not line.total or line.category_id.code not in category_codes:
                continue
            rule = line.salary_rule_id
            if rule.debit_account_id:
                key = (rule.debit_account_id, 'debit')
                amounts[key] = amounts.get(key, 0.0) + abs(line.total)
            if rule.credit_account_id:
                key = (rule.credit_account_id, 'credit')
                amounts[key] = amounts.get(key, 0.0) + abs(line.total)
        move_lines = []
        for (account, direction), amount in amounts.items():
    ...
    def _create_account_move(self):
        """Génère et poste l'écriture comptable du bulletin, si la société a
        activé les écritures de paie (company.hr_payroll_entries) et que des
        comptes débit/crédit sont configurés sur les règles salariales
        utilisées."""
        self.ensure_one()
        if not self.company_id.hr_payroll_entries:
            return
        move_lines = self._prepare_account_move_line_vals()
        if not move_lines:
            return
        journal = self.company_id.payroll_journal_id
        if not journal:
            raise UserError(
                _("Veuillez configurer un journal de paie pour la société "
                  "%s (Paramètres > Paie > Comptabilité).")
    ...
    def action_open_move(self):
        """Ouvre l'écriture comptable générée pour ce bulletin"""
        self.ensure_one()
        return {
    ...
    def action_view_payslip_pdf(self):
        """Génère le PDF du bulletin et l'ouvre directement dans le
        navigateur (visionneuse PDF intégrée), sans passer par l'aperçu
        d'impression Odoo."""
        self.ensure_one()
        report = self.env.ref(
            'hr_payroll_community.hr_payslip_new_report_action')
        pdf_content, _report_format = report._render_qweb_pdf(
            report.id, self.ids)
        attachment = self.env['ir.attachment'].create({
            'name': '%s.pdf' % (self.number or self.name),
            'type': 'binary',
    ...
    def action_refund_sheet(self):
        ...
        for payslip in self:
            ...
            copied_payslip.action_payslip_verify()
    ...
    def get_inputs(self, contracts, date_from, date_to):
        ...
        res += self._get_salary_attachment_inputs(contracts, date_from,
                                                   date_to)
        res += self._get_benefit_inputs(contracts, date_from, date_to)
        return res

    @api.model
    def _get_benefit_inputs(self, contracts, date_from, date_to):
        """Ajoute un input de bulletin par avantage en nature configuré sur
        le contrat, en scindant la part imposable et la part non
        imposable."""
        res = []
        for contract in contracts:
            for benefit in contract.benefit_ids:
                label = dict(benefit._fields['benefit_type'].selection).get(
                    benefit.benefit_type)
                if benefit.taxable_value:
                    res.append({
                        'name': _('%s (imposable)') % label,
                        'code': benefit.get_taxable_input_code(),
                        'amount': benefit.taxable_value,
                        'contract_id': contract.id,
                        'date_from': date_from,
                        'date_to': date_to,
                    })
                if benefit.non_taxable_value:
                    res.append({
                        'name': _('%s (non imposable)') % label,
                        'code': benefit.get_non_taxable_input_code(),
                        'amount': benefit.non_taxable_value,
                        'contract_id': contract.id,
                        'date_from': date_from,
                        'date_to': date_to,
                    })
    ...
    @api.model
    def _get_salary_attachment_inputs(self, contracts, date_from, date_to):
        """Ajoute un input de bulletin par retenue sur salaire (avance,
        pension alimentaire, saisie) active sur la période, pour l'employé
        de chaque contrat."""
        res = []
        for contract in contracts:
            ...
            for attachment in attachments:
                res.append({
                    'name': dict(attachment._fields['attachment_type']
                                 .selection).get(attachment.attachment_type),
                    'code': attachment.get_input_code(),
